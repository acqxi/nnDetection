...
from nndet.arch.heads.instance_segmenter import InstanceSegmenterType, BaseInstanceSegmenter
...
class RetinaUNetModule(LightningBaseModuleSWA):
    ...
    head_instance_segmenter_cls = BaseInstanceSegmenter
    ...
    def __init__(self, model_cfg: dict, trainer_cfg: dict, plan: dict,
        ...
        self.pre_trafo = Compose(
            ...
            Instances2Segmentation(
                ...
                map_key="instance_mapping",
                seg_key=
                "target_seg",  # Save semantic segmentation to separate key
                present_instances="present_instances",
            ))
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Prepare target_instance_seg if instance segmentation is enabled
        target_instance_seg = None
        if hasattr(self.model.head, 'instance_segmenter'
                   ) and self.model.head.instance_segmenter is not None:
            # Extract instance segmentation maps from the batch
            # batch['target'] contains the original instance segmentation map
            target_instance_seg = [
                batch['target'][i] for i in range(len(batch['target']))
            ]
        ...
        losses, _ = self.model.train_step(
            ...
            targets={
                ...
                "target_seg": batch['target_seg']
                [:, 0],  # Use semantic segmentation from separate key
                "target_instance_seg":
                target_instance_seg,  # Instance segmentation maps
            },
            evaluation=False,
            batch_num=batch_idx,
        )

        # Apply loss weighting according to the spec (0.5 for instance segmentation)
        if 'inst_seg' in losses:
            weighted_loss = (0.5 * losses.get('inst_seg', 0) +
                             0.25 * losses.get('reg', 0) +
                             0.25 * losses.get('cls', 0) +
                             0.25 * losses.get('seg_ce', 0) +
                             0.25 * losses.get('seg_dice', 0))
        else:
    ...
    def from_config_plan(
        ...
        **kwargs,
        ...
        # Build instance segmenter if enabled in config
        instance_segmenter = None
        if model_cfg.get("enable_instance_segmentation", False):
            instance_segmenter = cls._build_head_inst_segmenter(
                plan_arch=plan_arch,
                model_cfg=model_cfg,
                anchor_generator=anchor_generator,
            )
        ...
        head = cls._build_head(plan_arch=plan_arch,
                               ...
                               regressor=regressor,
                               instance_segmenter=instance_segmenter,
                               coder=coder)
    ...
    @classmethod
    def _build_head_inst_segmenter(
        cls,
        plan_arch: dict,
        model_cfg: dict,
        anchor_generator: AnchorGeneratorType,
    ) -> InstanceSegmenterType:
        """
        Build instance segmentation subnetwork for detection head
        ...
        Returns:
            InstanceSegmenterType: instance segmentation head instance
        """
        conv = Generator(cls.head_conv_cls, plan_arch["dim"])
        name = cls.head_instance_segmenter_cls.__name__
        kwargs = model_cfg.get('head_instance_segmenter_kwargs', {})

        logger.info(f"Building:: instance segmenter {name}: {kwargs}")
        instance_segmenter = cls.head_instance_segmenter_cls(
            conv=conv,
            in_channels=plan_arch["fpn_channels"],
            internal_channels=plan_arch["head_channels"],
            anchors_per_pos=anchor_generator.num_anchors_per_location()[0],
            num_levels=len(plan_arch["decoder_levels"]),
            output_size=(7, 28, 28),  # As specified in the requirements
            **kwargs,
        )
        return instance_segmenter
    ...
    def _build_head(
        ...
        coder: CoderType,
        instance_segmenter: Optional[InstanceSegmenterType] = None,
    ) -> HeadType:
        """
        ...
        Args:
            ...
            instance_segmenter: instance segmentation head (optional)
